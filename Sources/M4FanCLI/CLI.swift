...
struct CLI {
    ...
    func run() throws {
        ...
        case "doctor":
            try doctor()
        default:
            throw CLIError("Unknown command '\(parsed.command)'. Run m4fan help.", exitCode: 2)
        }
    }
    ...
    private func setFan() throws {
        ...
        stdoutLine("Use 'sudo .build/debug/m4fan auto --live --i-understand' to return to automatic control.")
    }
    ...
    private func printHelp() {
        stdoutLine(
            """
            m4fan - personal Apple Silicon fan-control CLI for Steven's M4 Pro Mac
            ...
            Dry-run write commands:
              m4fan set --fan 0 --percent 45
              m4fan set --fan 0 --rpm 3000
              m4fan auto
              m4fan curve --fan 0 --points 40:40,60:50 --once
