...
#   https://github.com/StevenACZ/MacGauge/releases/latest/download/appcast.xml
set -euo pipefail

cd "$(dirname "$0")/.."

APP_PATH="${APP_PATH:-dist/MacGauge.app}"
OUTPUT_DIR="${OUTPUT_DIR:-dist}"
REPO_URL="https://github.com/StevenACZ/MacGauge"
...
SPARKLE_BIN="$(ls -d .build/artifacts/sparkle/Sparkle/bin 2>/dev/null | head -1)"
if [[ -z "$SPARKLE_BIN" ]]; then
  echo "Sparkle tools not found under .build/artifacts." >&2
...
ZIP_NAME="MacGauge-v$VERSION.zip"
